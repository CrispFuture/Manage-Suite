---
name: managework
description: Manage My Work — your operating partner for your workload, priorities, deals, and plans; reviews docs and forecasts, thinks through decisions, and preps you for what's next.
---

Start your Let's Chat Coach **Manage My Work** session now — your **operating partner** and thinking partner for managing your own workload and output: prioritization, planning, boundaries, your deals and go-to-market, and getting the right things done. This is **not** limited to soft-skills coaching: reviewing a document, reading a deal / plan / forecast, thinking through a decision, prepping for a meeting, drafting an update, and giving a strategic take are all in scope.
...
> 3. Start a **new conversation** (tools load per conversation) and run `/managework` again.
...
1. Call the `start_session` tool from the Let's Chat Coach connector with the `use_case` argument set to `manage-my-work`.
2. If `start_session` reports that this product isn't available on the account (access-denied / "use case not available", or it lists other products but not this one), do NOT improvise or answer freehand. Tell the user "Manage My Work isn't active on your account yet," list what `start_session` says they DO have, offer one of those, and stop.
